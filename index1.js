...
class Board {
  ...
  // }
  handleCompeleteRow() {
    // Lọc các hàng chưa được lấp đầy bởi cùng một màu
    const lateGrid = board.grid.filter((row) => {
      return !row.every((col) => col === row[0]); // Kiểm tra nếu không phải tất cả các ô trong hàng đều cùng màu
    });

    // Tính số hàng đã hoàn thành (cùng màu)
    const completedRows = board.grid.length - lateGrid.length;

    // Tạo các hàng mới để thêm vào đầu lưới
    ...
    if (completedRows > 0) {
      // Cập nhật lại lưới
      board.grid = [...newRows, ...lateGrid];

      // Cập nhật điểm
      this.handleScore(completedRows * 10);
    }
  }
